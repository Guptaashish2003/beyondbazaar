...
export async function GET(request){
    ...
    try {
        ...
        const data ={totalSales:totalSales[0].totalSales,totalOrderByUser:totalOrderByUser[0].totalOrderByUser,totalReviews:totalReviews[0].totalReviews,totalDelivered:totalDelivered[0].totalDelivered,totalCancelled:totalCancelled[0].totalCancelled,totalPending:totalPending[0].totalPending,totalShipped:totalShipped[0].totalShipped}

        const DashBoardData  = await Dashboard.create({totalUsers,totalProducts,totalOrders,...data});
        // const DashBoardData  = await Dashboard.create({totalOrderByUser});

        
       return NextResponse.json({success:true,message:"DashBoard Data",data:DashBoardData},{status:200});
